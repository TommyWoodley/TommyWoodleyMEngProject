...
class MavrosOffboardSuctionMission():
    ...
    def swingPendelumViaPosition(self, centerX, centerY, centerZ):

        frequenzy = 20
        amplitude = 1
        rate = rospy.Rate(frequenzy)  # Hz

        PointInfront = [centerX, centerY+amplitude, centerZ]
        PointBehind = [centerX, centerY-amplitude, centerZ]
        nextTarget = PointBehind
        LastTarget = PointInfront
        vel = -0.5

        pendelumFrequenzy = 2.5
        numberOfSwings = 5
        ...
        paths = numberOfSwings*2-1
        totalTime = (pendelumFrequenzy * (numberOfSwings-0.5))
        frames = int(frequenzy * totalTime)
        framesOnePath = int(frames/paths)

        print("Nr of paths: ", paths, "toatal Time: ", totalTime, "lengthOnePath: ", framesOnePath, "Total nr Frames: ", frames)

        #for i in range(frames):

        while(not rospy.is_shutdown() and count < numberOfSwings):

            """if i>0 and i%framesOnePath==0:
                print("switch: ", i%framesOnePath, i , framesOnePath)
                if nextTarget == PointInfront:
                    nextTarget = PointBehind
                    vel *= -1
                else:
                    nextTarget = PointInfront
                    vel *= 1"""
            ...
            self.pos_target.position.x = nextTarget[0]
            self.pos_target.position.y = nextTarget[1]
            self.pos_target.position.z = nextTarget[2]

            self.pos_target.velocity.x = 0
            self.pos_target.velocity.y = vel
            self.pos_target.velocity.z = 0
            self.pos_target.yaw = 0
            ...
            self.saveDataToLogData(nextTarget[0], nextTarget[1], nextTarget[2])

            print(nextTarget, self.returnDifference(nextTarget))

            if self.returnDifference(LastTarget) > 2*amplitude-self.offset:
                print("switch!")
                count +=1
                vel *= -1
                attempts = 0
                if nextTarget == PointInfront:
                    ...
                    nextTarget = PointInfront
                    LastTarget = PointBehind

            if attempts > frequenzy*secondsWaiting: # assume conection is made -> can not reach target 
                rospy.loginfo("Connection made! continue ...")
                break
    ...
    def takeoff(self,x,y,z):
        ...
        while(not rospy.is_shutdown()):
            ...
            if self.is_at_position(self.offset,x, y, z, printOut=False):
    ...
    def run_complet_mission(self, xTarget = 0, yTarget = -2, zTarget = 1):
    ...
    def run_mission_no_approching(self, xTarget = 0, yTarget = -2, zTarget = 1):
...
if __name__ == '__main__':
    ...
    suction_mission.run_complet_mission(xTarget = 0.65, yTarget = 0.45, zTarget = 1.4)
    #suction_mission.run_pendlum_swing(xTarget = 0.65, yTarget = 0.45, zTarget = 1.4)
    #suction_mission.run_step1(xTarget = 0.65, yTarget = 0.45, zTarget = 1.4)
    #suction_mission.run_mission_no_approching(xTarget = 0.65, yTarget = 0.45, zTarget = 1.4)
    #suction_mission.auto_send_landing_pos_att()
    #rospy.spin()
