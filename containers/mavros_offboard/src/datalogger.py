...
class dataLogger(object):
        ...
        def plotFigure(self):
            ...
            cmap = plt.get_cmap('autumn') # matplotlib.cm.hot(np.abs(tot_v))
            ax1.scatter(self.x_pos[1:],self.y_pos[1:],self.z_pos[1:], c=cmap(1-np.abs(tot_v)/abs(max(tot_v))), edgecolor='none', label='Drone trajectory') #,color='blue')
            ax1.set_xlabel('x')
            ax1.set_ylabel('y')
            ax1.set_zlabel('z')
            ax1.set_xlim3d(-2,2)
